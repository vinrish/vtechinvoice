...
const totals = computed(() => {
  ...
  const tax = Math.round((subtotal * (Number(form.tax_rate) || 0)) ) / 100; // tax_rate is percent
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label class="mb-1 block text-sm">Issued At</label>
          <Input v-model="form.issued_at" type="date" />
          <div v-if="form.errors.issued_at" class="text-sm text-red-600 mt-1">{{ form.errors.issued_at }}</div>
        </div>
        <div>
          <label class="mb-1 block text-sm">Due At</label>
          <Input v-model="form.due_at" type="date" />
          <div v-if="form.errors.due_at" class="text-sm text-red-600 mt-1">{{ form.errors.due_at }}</div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        <Button :disabled="form.processing" @click="submit">Create Invoice</Button>
